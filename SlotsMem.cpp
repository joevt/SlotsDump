...
void DumpMem(uint8_t slot, Ptr start, int32_t len)
{
	FILE * dumpFile = NULL;
	char fileName[100];
	...
	snprintf(fileName, sizeof(fileName), "SlotMem_%02" PRIX8 ".bin", slot);
	dumpFile = fopen(fileName, "wb");
	if (!dumpFile)
		goto error;

	printf("\nSaving slot memory to file \"%s\"...\n", fileName);
	fwrite(start, 1, (uint32_t)len, dumpFile);
	goto done;
error:
	printf("DumpROM error slot:%02" PRIX8 "\n", slot);
done:
...
void WriteSInfoRecordAndFHeaderRec(uint8_t slot, bool write_progress)
{
	...
	if (slot == 0) {
		len = fh.fhLength;
		romstart = CalcAddr(gTopOfRom, -len);
		DumpROM(slot, romstart, len);
	} else {
		if ((uint32_t)si->siROMAddr > 0xF0000000)
			len = 16L*1024*1024;
		else
			len = 1L*1024*1024;
		DumpMem(slot, gTopOfRom - len, len);
	}
